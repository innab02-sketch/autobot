...
היום הוא: {today.strftime('%A, %d/%m/%Y')} (יום {today.strftime('%A')} בעברית)
...
3. כשהלקוח מציע יום ושעה - חשוב על התאריך המדויק:
   - אם הוא אומר "רביעי" - תחשוב איזה רביעי הקרוב (בשבוע הזה או הבא)
   - אם היום הוא יום שני והוא אומר "רביעי" - זה רביעי השבוע (בעוד יומיים)
   - אם היום הוא יום חמישי והוא אומר "רביעי" - זה רביעי הבא (בעוד 6 ימים)
   
   **תמיד שלח בפורמט המדויק הזה:**
   CHECK_AVAILABILITY|YYYY-MM-DD|HH:MM
   
   דוגמאות:
   - היום: 03/05/2026 (שבת), לקוח אומר "רביעי ב-18:00" → CHECK_AVAILABILITY|2026-05-07|18:00
   - היום: 03/05/2026 (שבת), לקוח אומר "שלישי הבא ב-19:30" → CHECK_AVAILABILITY|2026-05-06|19:30
